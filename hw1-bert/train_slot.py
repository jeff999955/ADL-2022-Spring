...
def train(model, device, data_loader, criterion, optimizer):
    ...
    model.train()
    for tokens, tags, length, id in tqdm(data_loader):
        ...
        acc = ((logits.argmax(dim=-1) == tags).sum(dim=1)
               == args.max_len).cpu().int().numpy()
        train_loss.append(loss.item())
        train_accs.extend(acc)
...
def main(args):
    ...
    datasets = {
        split: STDataset(split_data, vocab, tags2idx, args.max_len)
...
if __name__ == "__main__":
    ...
    print(args)
    wandb.login()
    wandb.init(project="slot tagging")
